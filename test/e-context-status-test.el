...
(ert-deftest e-context-status-test-text-uses-status-context-purpose ()
  "Shared status text requests optional status context for estimates."
  (let* ((store (e-session-store-create))
         (harness (e-harness-create
                   :backend (e-backend-fake-create :items nil)
                   :sessions store
                   ...
                   '(:model "gpt-5.5" :reasoning-effort "high")))
         seen-purpose)
    (e-session-create store :id "status-purpose")
    (cl-letf (((symbol-function 'e-harness-context)
               (lambda (_harness _session-id _turn-id context-purpose)
                 ...
                 '(:messages ((:role user :content "status purpose"))
