...
- `e-skill-spec-create`, `e-capability-with-skills-create`, and `e-skills-uri-for-name`: construction-time skill specs that build ordinary capabilities with compact instruction references and conventional `e://<capability>/skills/<skill-name>` resources. `e-skill-create` and `e-skills-register` remain compatibility-only helpers; the harness does not consume a skill catalog.
