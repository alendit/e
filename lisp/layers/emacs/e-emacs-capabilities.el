...
(defconst e-elisp-eval-instructions
  (string-join
   '("When using run_elisp, evaluated code may compose currently active tools and actions through context-bound APIs:"
     ...
     ""
     "Use nested tool calls only for cheap composition. Long or async-backed tools must run as top-level tools so their Work lifecycle can stream progress and settle outside this eval."
     ""
     "Use this when several cheap tool calls are needed to compute one answer, later calls depend on earlier results, many similar resources need the same read/search/edit operation, or the result should be filtered, grouped, or summarized before returning."
     ""
